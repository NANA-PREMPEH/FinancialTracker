...
class TestRuleExecution:
    ...
    def test_automation_engine_imports(self, app, db):
        """Test that automation engine module can be imported."""
        from app.automation_engine import execute_rules
        assert callable(execute_rules)

    def test_rule_creation_and_retrieval(self, app, test_user, db):
        """Test rule can be created and retrieved."""
        with app.app_context():
            rule = AutomationRule(
                user_id=test_user.id,
                name='Test Rule',
            ...
            found = AutomationRule.query.filter_by(user_id=test_user.id).first()
